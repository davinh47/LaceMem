...
> **Current branch:** `context-aware` — this checkout runs the **context-aware** raw-memory path (`RawMemoryManager` with pending topic/time; see `prompts/raw_memory_manager.py`). Use `git checkout main` for the standard one-shot raw ingest. Postgres defaults are still **`LaceMem`** (see [Git branches](#git-branches)).
