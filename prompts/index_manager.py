...
- Do NOT drop user-relevant information when splitting. If you cannot encode a detail as part of an atomic fact,
  create an additional atomic fact to preserve it.
- If any candidate SPO would have an overlong predicate/object or would embed multiple relations (i.e., an object that
  could be split into more than one subject–predicate–object), you MUST split it into multiple atomic items. Otherwise, you MAY keep it as a single item.
...
- HOWEVER, avoid nested or compositional objects that themselves contain full SPO meaning.
  If the object contains multiple facts (who/what/where/when/type/name), you MUST factor them into separate items.
  For example:
    INVALID: (subject=X, predicate=watched, object="a movie called Wizard of Oz")
    VALID: split into:
       - (subject=X, predicate=watched, object=Wizard of Oz)
       - (subject=Wizard of Oz, predicate=is, object=movie)
...
(subject=Over the Rainbow, predicate=is, object=theme song)
